...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        if path in self.cacheTable.keys() and not self.cacheTable.expired(path):
            # return self.cacheTable.getHeaders(path), self.cacheTable.getBody(path)
            yield self.cacheTable.getHeaders(path), self.cacheTable.getBody(path)
            # raise StopIteration
        else:
            ...
            if response is None:
                ...
                # body = bytearray(response.read())
                self.cacheTable.setHeaders(path, headers)
                while True:
                    cntByte = response.readinto(buffer)
                    if cntByte == 0:
                        raise StopIteration
                    else:
                        self.cacheTable.appendBody(path, buffer[0: cntByte])
                        yield headers, buffer
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    @trace
    def sendHeaders(self, headers: List[Tuple[str, str]]):
    ...
    def do_GET(self):
        ...
        # if headers is None:
        #     self.send_error(HTTPStatus.NOT_FOUND, "''Oops:-), File Not Found!'")
        #     self.end_headers()  # clear headers' queue
        # else:
        ...
        '''
            process with buffer
        '''
        touch = self.server.touchItem(self.path)
        headers, body = next(touch)
        if headers is None:
            self.send_error(HTTPStatus.NOT_FOUND, "''Oops:-), File Not Found!'")
            self.end_headers()  # clear headers' queue
        else:
