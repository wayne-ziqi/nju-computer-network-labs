...
class PySwitchTopo(Topo):
    ...
    def __init__(self, args):
        ...
        self.addLink("client1", "hub1", bw=10, delay="100ms")
        self.addLink("server1", "hub1", bw=10, delay="100ms")
        self.addLink("hub1", "hub2", bw=10, delay="100ms")
        self.addLink("client2", "hub2", bw=10, delay="100ms")
        self.addLink("server2", "hub2", bw=10, delay="100ms")
...
def setup_addressing(net):
    for node, config in nodes.items():
        ...
        if node == "client1" or node == "server1":
            set_ip(net, node, "hub1", config["ip"])
        elif node  == "client2" or node == "server2":
            set_ip(net, node, "hub2", config["ip"])
