...
def test_hub():
    ...
    s.expect(
        PacketOutputEvent("eth0", testpkt, "eth2", testpkt,"eth3", testpkt, display=Ethernet),
        ("The Ethernet frame with a broadcast destination address should be "
         "forwarded out ports eth0, eth2, eth3")
    )
    ...
    s.expect(
        PacketOutputEvent("eth0", reqpkt, "eth1",reqpkt, "eth3", reqpkt, display=Ethernet),
        ("the packet should be broadcast to all other interfaces")
    )

    # test5: a packet sent to the destination MAC that doesn't exist in the entry
    reqpkt = new_packet(
        "20:00:00:00:00:01",
        "40:00:00:00:00:01",
        "192.168.1.101",
    ...
    s.expect(
        PacketOutputEvent("eth0", reqpkt, "eth1", reqpkt, "eth3", reqpkt, display=Ethernet),
        ("the packet should be flooded to all other interfaces")
    )

    # test6: resend a packet after the topology changed
    resppkt = new_packet(
        "30:00:00:00:00:02",
        "20:00:00:00:00:01",
        '172.16.42.2',
